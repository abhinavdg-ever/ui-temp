...
- **Ad-hoc scripts** — see [`07-ad-hoc/ADHOC_README.md`](07-ad-hoc/ADHOC_README.md) to organize images + OCR into `pages/` / `ocr/`
- **Docker** — backend `:3000`, frontend `:3001` (nginx proxies `/api`)

## Monorepo layout

```
AI Project POC / this repo/
  05-imaging-ui/          ← UI app (.env, backend, frontend, data/folders)
  06-postgres-db/         ← DDL, manifest CSVs, db-insert scripts
  07-ad-hoc/              ← organize_pages / organize_ocr helpers
```

## Local folder layout

```
05-imaging-ui/data/folders/<folder_name>/
  pages/1.jpg
  pages/2.jpg
...
```bash
# Backend
cd 05-imaging-ui/backend && source .venv/bin/activate
uvicorn app.main:app --reload --host 127.0.0.1 --port 8002

# Frontend
cd 05-imaging-ui/frontend && npm run dev
```
