...
DEFAULT_MODEL_PATH = SCRIPT_DIR / "models" / "image_type_classification.pkl"
...
def load_model(model_path: Path | None = None) -> Any:
    ...
    try:
        ...
        raise SystemExit(
            "joblib is required: pip install -r requirements.txt"
        ) from exc
...
def extract_ocr_features(image_bytes: bytes) -> list[float]:
    ...
    try:
        ...
        raise SystemExit(
            "pytesseract is required (and a system Tesseract install)"
        ) from exc
